...
function setupDragAndDrop(card, entryId) {
  ...
  card.addEventListener('drop', (e) => {
    ...
    if (draggedId && draggedId !== entryId) {
      const rect = card.getBoundingClientRect();
      const next = (e.clientY - rect.top) / (rect.bottom - rect.top) > 0.5;
      reorderEntries(draggedId, entryId, next);
    }
  });
}
...
function setupTouchDrag(card, entryId) {
  card.addEventListener('touchstart', (e) => {
    ...
    touchDragState.startIndex = state.entries.findIndex(entry => entry.id === entryId);
    touchDragState.elements = Array.from(elNominalList.querySelectorAll('.nominal-card'));
  ...
  card.addEventListener('touchmove', (e) => {
    ...
    touchDragState.elements.forEach((el) => {
      if (el === card) return;
  ...
  card.addEventListener('touchend', (e) => {
    ...
    touchDragState.elements.forEach((el) => {
      ...
      if (currentY > rect.top && currentY < rect.bottom) {
        ...
        insertAfter = currentY > (rect.top + rect.height / 2);
      }
      el.classList.remove('drag-over-before', 'drag-over-after');
    });
