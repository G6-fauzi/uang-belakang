...
function renderApp() {
  ...
  // Also update nav visibility because content size might have changed
  // Use setTimeout to allow DOM to recalculate heights
  setTimeout(() => {
...
function switchTab(tabId) {
  ...
  document.getElementById(tabId === 'home' ? 'navHome' : 'navHistory').classList.add('active');
  
  const elHome = document.getElementById('viewHome');
  const elHistory = document.getElementById('viewHistory');
  
  // Make both fully visible for smooth sliding transition
  elHome.style.height = '';
  elHome.style.overflow = '';
  elHistory.style.height = '';
  elHistory.style.overflow = '';
  
  if (tabId === 'home') {
    elViewsSlider.style.transform = 'translateX(0)';
    setTimeout(() => {
      if (document.getElementById('navHome').classList.contains('active')) {
        elHistory.style.height = '0px';
        elHistory.style.overflow = 'hidden';
        if (typeof updateNavVisibility === 'function') updateNavVisibility();
      }
    }, 350);
  } else {
    ...
    setTimeout(() => {
      if (document.getElementById('navHistory').classList.contains('active')) {
        elHome.style.height = '0px';
        elHome.style.overflow = 'hidden';
        if (typeof updateNavVisibility === 'function') updateNavVisibility();
      }
    }, 350);
  }
...
function updateNavVisibility() {
  ...
  // If content is too short to scroll meaningfully, always show the nav
  if (maxScroll <= 10) {
    bottomNav.classList.remove('nav-hidden');
  } else if (scrollArea.scrollTop <= 10) {
...
scrollArea.addEventListener('scroll', updateNavVisibility);
